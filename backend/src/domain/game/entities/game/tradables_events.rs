...
impl Game {
    ...
    pub fn add_tradable(&mut self, tradable_id: Uuid, name: String, initial_value: f64) -> Result<(), GameError> {
        ...
        self.tradables.push(Tradable::new(
            ...
            name,
            initial_value,
            self.players.iter().map(|p| p.id()).cloned().collect::<Vec<Uuid>>()
        ));
        Ok(())
    }

    pub fn remove_tradable(&mut self, tradable_id: Uuid) -> Result<(), GameError> {
    ...
    pub fn change_player_tradable_value(&mut self, player_id: Uuid, tradable_id: Uuid, new_value: f64) -> Result<(), GameError> {
    ...
    pub fn send_tradable(&mut self, from_id: Uuid, to_id: Uuid, tradable_id: Uuid, amount: f64) -> Result<(), GameError> {
        if let Some(tradable) = self.tradables.iter_mut().find(|t| t.id() == &tradable_id) {
...
mod tests {
    ...
    fn test_change_player_tradable_value_nonexistent_tradable() {
        ...
        let tradable_id = Uuid::new_v4(); // Not added to the game
