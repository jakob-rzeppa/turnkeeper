...
pub enum GameInstanceError {
    ...
    #[error("Player not found: {0}")] PlayerNotFound(String),

    #[error("Stat {0} not found")] StatNotFound(String),
    #[error("Player stat {0} not found")] PlayerStatNotFound(String),
    #[error("Player {player_name} not found in stat {stat_name}")] PlayerInStatNotFound {
        player_name: String,
        stat_name: String,
    },

    #[error("Invalid player order provided: {0:?}")] InvalidPlayerOrder(Vec<String>),
