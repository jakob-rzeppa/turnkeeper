...
use crate::infrastructure::http::game_instance::{
    game_instances_delete,
    game_instances_get_metadata_by_game_id,
    game_instances_post,
};
...
pub fn get_routes(state: AppState) -> Router<AppState> {
    Router::new()
        .route(
            "/games",
            get(games_get).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
            "/games/{id}",
            get(games_get_by_id).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
            "/games",
            post(games_create).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
            "/games/{id}/source-code",
            patch(games_update_source_code).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
            "/games/{game_id}/check",
            get(games_check_source_code).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
            "/games/{id}",
            delete(games_delete).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
        ...
        .route(
            "/games/{game_id}/instances",
            post(game_instances_post).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
            "/games/{game_id}/instances/{instance_id}",
            delete(game_instances_delete).route_layer(
                middleware::from_fn_with_state(state.clone(), auth_middleware)
            )
        )
        .route(
            "/users",
            get(list).route_layer(middleware::from_fn_with_state(state.clone(), auth_middleware))
        )
        // User routes
