...
use crate::{
    application::{
        common::channels::{
            ...
            targeted_broadcast::TargetedBroadcastReceiver,
        },
...
impl GameSessionManager {
            ...
            loop {
                tokio::time::sleep(std::time::Duration::from_secs(5)).await;
                {
                    ...
                    for (game_id, (_session, user_ids)) in sessions_guard.iter() {
                        println!(
                            "[LOGGER] -> Game ID: {}, Connected Users: {}",
                            game_id,
                            user_ids.len()
                        );
                    }
    ...
    pub async fn connect_to_session(
        ...
        (MpscChannelSender<IncomingMessageDto>, TargetedBroadcastReceiver<Id, OutgoingMessageDto>),
    ...
    > {
        ...
        if !sessions_guard.contains_key(game_id) {
            let new_session = GameSession::spawn(
                game_id.clone(),
                self.game_instance_repo.clone()
            ).await?;
        ...
        if let Some(session) = sessions_guard.get_mut(game_id) {
            session.1.remove(
                session.1
                    ...
                    .unwrap_or_else(||
                        panic!("Disconnect called for user_id {} which is not in the session's connected users list", user_id)
                    )
            );
