...
impl Parsable for IfStatement {
    ...
    fn parse(ts: &mut TokenStream, source_code: &str) -> Result<Self, ParsingError> {
        ...
        expect_token!(ts, TokenVariant::OpenBrace, "Expected '{' after condition in if statement");
        ...
        while is_token!(ts, TokenVariant::Else) {
            ...
            if is_token!(ts, TokenVariant::If) {
                ...
                else_if_branches.push(
                    ElseIfBranch::new(else_if_condition, else_if_then_branch, else_pos)
                );
            } else {
                expect_token!(
                    ts,
...
mod tests {
            ...
            IfStatement::new(
                Expression::Binary(
                    BinaryExpression::new(
                        Expression::Atom(
                            ExpressionAtom::Variable("x".to_string(), Position::new(0, 3))
                        ),
                        BinaryOperator::GreaterThan,
                        Expression::Atom(
                            ExpressionAtom::Literal(Value::Float(0.0), Position::new(0, 7))
                        ),
                        Position::new(0, 3)
                    )
                ),
                vec![
                    Statement::Assignment(
                        AssignmentStatement::new(
                            "y".to_string(),
                            Expression::Atom(
                                ExpressionAtom::Literal(Value::Float(1.0), Position::new(0, 17))
                            ),
                            Position::new(0, 13)
                        )
                    )
        ...
        assert_eq!(
            ...
            IfStatement::new(
                ...
                vec![
                    ElseIfBranch::new(
                        Expression::Atom(
                            ExpressionAtom::Literal(Value::Bool(true), Position::new(0, 21))
                        ),
                        vec![
                            Statement::Assignment(
                                AssignmentStatement::new(
        ...
        assert_eq!(
            ...
            IfStatement::new(
                ...
                vec![
                    ElseIfBranch::new(
                        Expression::Atom(
                            ExpressionAtom::Variable("x".to_string(), Position::new(0, 21))
                        ),
                        vec![],
                        Position::new(0, 13)
                    )
    ...
    fn test_multiple_else_if_branches() {
        let (mut ts, source_code) = test_token_stream!(
            "if false { } else if x { } else if y { } else { z = 42; }"
        );
        ...
        assert_eq!(
            ...
            IfStatement::new(
                ...
                vec![
                    ...
                    ElseIfBranch::new(
                        Expression::Atom(
                            ExpressionAtom::Variable("y".to_string(), Position::new(0, 35))
                        ),
                        vec![],
                        Position::new(0, 27)
                    )
