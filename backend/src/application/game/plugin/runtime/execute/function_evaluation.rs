use crate::application::game::plugin::{parser::old_abstract_syntax_tree::expression::FunctionCall, runtime::{RuntimeEnvironment, memory::VariableValue}};
...
impl RuntimeEnvironment {
    ...
    pub fn evaluate_function(&mut self, function_call: &FunctionCall) -> Result<VariableValue, String> {
        let function_identifier = function_call.identifier.to_string();

        match function_identifier.as_str() {
            "print" => {
                let arguments = function_call.arguments.iter().map(|arg| self.evaluate_expression(arg)).collect::<Result<Vec<VariableValue>, String>>()?;
...
#[cfg(test)]
mod tests {
    use crate::application::game::plugin::{common::Position, parser::old_abstract_syntax_tree::{expression::{Expr, ExprAtom, Literal}, identifier::Identifier}};
