    ...
    ($($variant:expr),*) => {
        {
            let tokens = vec![$($variant),*]
                .into_iter()
                .enumerate()
                .map(|(i, variant)| crate::application::game::plugin::lexer::token::Token {
                    variant,
                    pos: crate::application::game::plugin::common::Position::new(i, 0),
                })
                .collect::<Vec<_>>();

            crate::application::game::plugin::parser::abstract_syntax_tree::TokenStream::new(tokens)
        }
    };
}
