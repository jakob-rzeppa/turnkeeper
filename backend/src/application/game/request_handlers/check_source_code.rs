...
impl GameRequestHandler {
    pub async fn check_source_code(
        ...
        match parsing_result {
            Ok(projections) =>
                Ok(CheckSourceCodeResponse::Valid {
                    game_stats: projections.game_stats
                        .into_iter()
                        .map(|s| s.get_metadata_projection())
                        .collect(),
                    player_stats: projections.player_stats
                        .into_iter()
                        .map(|s| s.get_metadata_projection())
                        .collect(),
                    actions: projections.actions
                        ...
                        .collect(),
                    pages: projections.pages
                        .into_iter()
                        .map(|p| p.get_metadata_projection())
                        .collect(),
                }),
            Err(error) =>
                Ok(CheckSourceCodeResponse::Invalid {
                    errors: vec![error],
                }),
        }
    }
