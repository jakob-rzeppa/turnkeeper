...
pub enum GameInstanceApplicationError {
    ...
    #[error("Game instance with id {0} not found")] NotFound(Id),
    #[error("Game with id {0} for creating game instance not found")] GameNotFound(Id),
    #[error("Error parsing game: {0}")] ParsingError(#[from] ParsingError),
}
