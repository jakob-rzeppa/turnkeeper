...
mod user_websocket_functional_test {
    use crate::common::user_game_connection::{user_ws_connect, user_ws_ticket_request};
    ...
    async fn ws_ticket_for_existing_game_returns_url() {
        ...
        let resp = user_ws_ticket_request(&addr, &user_token, &game_id).await;
    ...
    async fn two_different_users_can_get_tickets_for_same_game() {
        ...
        let resp1 = user_ws_ticket_request(&addr, &user_token_1, &game_id).await;
        ...
        let resp2 = user_ws_ticket_request(&addr, &user_token_2, &game_id).await;
    ...
    async fn ws_connect_with_valid_ticket_succeeds() {
        ...
        let mut ws_stream = user_ws_connect(&ws_url)
            .await
            .expect("Failed to connect to WebSocket with valid ticket");
    ...
    async fn ws_connect_with_invalid_ticket_closes_connection() {
        ...
        let mut ws_stream = user_ws_connect(&ws_url)
            .await
            .expect("Failed to connect to WebSocket with invalid ticket (should connect but then be closed by server)");
    ...
    async fn ws_ticket_twice_without_connecting_returns_conflict() {
        ...
        let resp1 = user_ws_ticket_request(&addr, &user_token, &game_id).await;
        assert_eq!(resp1.status(), StatusCode::OK);

        // Second ticket request while first is still pending – should fail
        let resp2 = user_ws_ticket_request(&addr, &user_token, &game_id).await;
        assert_eq!(resp2.status(), StatusCode::CONFLICT);
    }

    #[tokio::test]
    async fn ws_ticket_for_nonexistent_game_returns_not_found() {
        let addr = spawn_app().await;
        let user_token = user_register(&addr, "hannah", "test").await.unwrap().token;

        let resp = user_ws_ticket_request(&addr, &user_token, "00000000-0000-0000-0000-000000000000").await;
        assert_eq!(resp.status(), StatusCode::NOT_FOUND);
    }

    #[tokio::test]
    async fn ws_different_users_can_connect_to_same_game() {
        let addr = spawn_app().await;
        let gm_token = gm_login(&addr).await.unwrap().token;
        let game_id = create_game(&addr, &gm_token, "Shared Game").await;
        let user_token_1 = user_register(&addr, "ian", "test").await.unwrap().token;
        let user_token_2 = user_register(&addr, "julia", "test").await.unwrap().token;

        // User 1 gets a ticket and connects
        let resp1 = user_ws_ticket_request(&addr, &user_token_1, &game_id).await;
        assert_eq!(resp1.status(), StatusCode::OK);
        let body1: Value = resp1.json().await.unwrap();
        let url1 = body1["url"].as_str().unwrap();
        let mut ws_stream_1 = user_ws_connect(url1)
            .await
            .expect("User 1 failed to connect to WebSocket");

        // User 2 gets a ticket and connects
        let resp2 = user_ws_ticket_request(&addr, &user_token_2, &game_id).await;
        assert_eq!(resp2.status(), StatusCode::OK);
        let body2: Value = resp2.json().await.unwrap();
        let url2 = body2["url"].as_str().unwrap();
        let mut ws_stream_2 = user_ws_connect(url2)
            .await
            .expect("User 2 failed to connect to WebSocket");

        // Both users should receive the initial game state
        use futures_util::StreamExt;
        for (ws_stream, username) in [(&mut ws_stream_1, "ian"), (&mut ws_stream_2, "julia")] {
            let msg = tokio::time::timeout(std::time::Duration::from_secs(5), ws_stream.next())
                .await
                .expect("Timed out waiting for initial game state")
                .expect("Stream ended unexpectedly")
            ...
            match msg {
                ...
                other => panic!("Expected text message with game state for user {username}, got: {other:?}"),
            }
        }
    }
}